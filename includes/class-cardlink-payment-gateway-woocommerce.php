...
class Cardlink_Payment_Gateway_Woocommerce extends WC_Payment_Gateway {
	...
	function generate_cardlink_form( $order_id ) {
		...
		$form_data_string = implode( "", $form_data_array );
		$form_data        = iconv( mb_detect_encoding($form_data_string, mb_detect_order(), true), "UTF-8", $form_data_string) . $form_secret;
		...
		foreach ( $form_data_array as $key => $value ) {
			$encoded_value = iconv( mb_detect_encoding($value, mb_detect_order(), true), "UTF-8", $value);
			$html .= '<input type="hidden" id ="' . $key . '" name ="' . $key . '" value="' . $encoded_value . '"/>';
		}

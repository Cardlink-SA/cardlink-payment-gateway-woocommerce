<?php
/**
 * Cardlink Google Pay Direct Helper
 * Provides configuration accessors, URL builders, XID calculation, MPI signing,
 * and transaction info management for Google Pay Direct payments.
 */
class CardlinkGooglePayHelper
{
    ...
    {
        return self::getConfig('payment_cardlink_googlepay_merchantid');
    }

    public static function getSharedSecret()
    {
        return self::getConfig('payment_cardlink_googlepay_sharedSecret');
    }
    ...
    {
        return self::getConfig('payment_cardlink_googlepay_mode') ?: 'sandbox';
    }
    ...
    /**
     * Build query string for loading googlepaydirect.js.
    ...
    /**
     * Calculate 3DS XID from VPOS transaction fields.
     * Format: "VPOS"(4) + padCutLeft(txId,7)(7) + "-"(1) + padCutLeft(trExtId,6)(6) + padCutLeft(mpiCounts,2)(2) = 20 bytes
     * Then Base64-encoded.
     */
    ...
    /** Persist in-memory map to PHP session */
    public static function persistToSession()
    {
        if (session_status() !== PHP_SESSION_ACTIVE) {
            @session_start();
        }
        $_SESSION['cardlink_gpay_transaction_map'] = self::$transactionMap;
    }

    /** Restore in-memory map from PHP session */
    public static function restoreFromSession()
    {
        if (session_status() !== PHP_SESSION_ACTIVE) {
            @session_start();
        }
        if (isset($_SESSION['cardlink_gpay_transaction_map']) && is_array($_SESSION['cardlink_gpay_transaction_map'])) {
            self::$transactionMap = $_SESSION['cardlink_gpay_transaction_map'];
        }
    }

    /** Persist all in-memory entries to file cache (survives cross-domain MPI redirect) */
    ...
    {
        $cacheDir = defined('DIR_CACHE') ? DIR_CACHE . 'cardlink_gpay/' : sys_get_temp_dir() . '/cardlink_gpay/';
        if (!is_dir($cacheDir)) {
